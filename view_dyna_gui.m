...
% 振动数据查看界面
% - 支持读取 .vna/.mat/.txt/.dat/.csv/.xlsx
% - 支持按文件+通道单选或多选绘图
% - 支持时域、PSD、累计谱、传递率和相干性显示
% - 支持时域低通/高通滤波以及地基振动分析

% 根据屏幕分辨率计算初始窗口尺寸（自适应，不固定）
...
panel = uipanel('Parent', fig, 'Title', '控制区', 'Units', 'pixels', 'Position', [15 15 360 890]);
...
ddPsdSource = uicontrol('Parent', panel, 'Style', 'popupmenu', ...
    'String', {'时段计算 (periodogram)', 'VNA 原始结果'}, ...
    'Value', 2, ...
    'BackgroundColor', 'w', ...
...
% 左侧折叠分组：标题按钮 + 内容面板，保持 MATLAB 2016 兼容
...
tabMain = uitab('Parent', tabRight, 'Title', '主界面');
tabFoundation = uitab('Parent', tabRight, 'Title', '地面振动');
try
...
title(axMain3, '传递率 (dB)');
xlabel(axMain3, '频率 (Hz)');
ylabel(axMain3, 'dB');
...
title(axFoundVib, '地面振动（三分之一倍频程）');
xlabel(axFoundVib, '三分之一倍频程中心频率 [Hz]');
ylabel(axFoundVib, '均方根速度 [um/s]');
set(axFoundVib, 'XScale', 'log', 'YScale', 'log');
grid(axFoundVib, 'on');

axFoundStiff = axes('Parent', tabFoundation, 'Units', 'pixels', 'Position', [20 235 1030 215], 'Box', 'on');
title(axFoundStiff, '动态刚度');
xlabel(axFoundStiff, '频率 [Hz]');
ylabel(axFoundStiff, '幅值 [N/m]');
...
title(axFoundCoh, '相干性');
xlabel(axFoundCoh, '频率 [Hz]');
ylabel(axFoundCoh, '相干性');
...
set(btnFig1, 'Callback', @(~, ~) onOpenAxisFigure(axMain1, '图窗 1'));
set(btnFig2, 'Callback', @(~, ~) onOpenAxisFigure(axMain2, '图窗 2'));
set(btnFig3, 'Callback', @(~, ~) onOpenAxisFigure(axMain3, '图窗 3'));
set(btnExport1, 'Callback', @(~, ~) onExportAxisData(axMain1, '图窗 1'));
set(btnExport2, 'Callback', @(~, ~) onExportAxisData(axMain2, '图窗 2'));
set(btnExport3, 'Callback', @(~, ~) onExportAxisData(axMain3, '图窗 3'));
...
% 绑定窗口尺寸变化回调，并执行一次初始布局
set(fig, 'ResizeFcn', @onResize);
onResize();

    % 自适应布局：根据窗口尺寸重新排布左侧控件和右侧图窗
    function onResize(~, ~)
        ...
        set(btnDataGroup, 'String', getGroupTitle('数据', isDataGroupExpanded), ...
            'Value', double(isDataGroupExpanded));
        set(btnMainGroup, 'String', getGroupTitle('主处理', isMainGroupExpanded), ...
            'Value', double(isMainGroupExpanded));
        set(btnPlotGroup, 'String', getGroupTitle('绘图区', isPlotGroupExpanded), ...
    ...
    function onLoadFolder(~, ~)
        ...
        if isempty(fileList)
            showAlertCompat(fig, '所选文件夹中没有找到支持的数据文件。', '加载文件夹');
            return;
        end
    ...
    function finalizeLoadResult(app, loadedNow, failedNow, lastErr, srcLabel)
        ...
        if isempty(app.files)
            showAlertCompat(fig, '没有文件加载成功。', '加载失败');
            if ~isempty(lastErr)
            set(lblStatus, 'String', ['状态: 加载失败 | ' lastErr]);
            else
            set(lblStatus, 'String', '状态: 加载失败');
            end
            return;
        end
    ...
    function onExportAxisData(sourceAx, fallbackTitle)
        ...
        if isempty(lineHandles)
            showAlertCompat(fig, '当前图像为空，请先绘图。', '提示');
            return;
        end
        ...
        [saveFile, savePath, filterIndex] = uiputfile( ...
            {'*.dat', 'DAT 文件 (*.dat)'; '*.mat', 'MAT 文件 (*.mat)'; '*.xlsx', 'Excel 文件 (*.xlsx)'}, ...
            '导出图窗数据', defaultName);
        if isequal(saveFile, 0) || isequal(savePath, 0)
            set(lblStatus, 'String', '状态: 已取消导出图窗数据');
            return;
        end
        ...
        if ~ok
            showAlertCompat(fig, errMsg, '导出失败');
            return;
        end
        set(lblStatus, 'String', sprintf('状态: 图窗数据已导出到 %s', outPath));
    end
    function onPlot(~, ~)
        app = getappdata(fig, 'app');
        if ~app.loaded || isempty(app.files)
        showAlertCompat(fig, '请先加载数据。', '提示');
            return;
        end
        ...
        selectedSeries = getSelectedSeries(app.series, getSelectedListLabels(lstData));
        if isempty(selectedSeries)
        showAlertCompat(fig, '请至少选择一个数据项（文件+通道）。', '提示');
            return;
        end

        [timeWindow, rangeErr] = parseTimeRangeInputs(edtTStart, edtTEnd);
        if ~isempty(rangeErr)
            showAlertCompat(fig, rangeErr, '时间范围错误');
            return;
        end
        ...
        if keepExisting
            set(lblStatus, 'String', sprintf('状态: 保持开启，已追加 %d 个数据项%s', numel(selectedSeries), detailText));
        else
            set(lblStatus, 'String', sprintf('状态: 已绘制 %d 个选中数据项%s', numel(selectedSeries), detailText));
        end
    end
...
% 将指定轴当前内容复制到单独 Figure，便于保存图片
    function onOpenAxisFigure(sourceAx, fallbackTitle)
        ...
        set(lblStatus, 'String', sprintf('状态: 已在独立图窗中打开“%s”', figName));
    end

    function onOpenFoundationVibFigure(~, ~)
        if countLineLikeChildren(axFoundVib) == 0
            showAlertCompat(fig, '地面振动图为空，请先绘图。', '提示');
            return;
        end
        figName = getAxisExportTitle(axFoundVib, '地面振动');
    ...
    function onOpenFoundationStiffCohFigure(~, ~)
        ...
        if ~hasStiff && ~hasCoh
            showAlertCompat(fig, '地基刚度/相干性图为空，请先绘图。', '提示');
            return;
        end
        cloneTwoAxesToFigure(axFoundStiff, axFoundCoh, '动态刚度 + 相干性', 'northwest');
        set(lblStatus, 'String', '状态: 已在同一图窗中打开地基刚度/相干性');
    end


    function onExportFoundationVibData(~, ~)
        onExportAxisData(axFoundVib, '地面振动');
    end
    ...
    function onExportFoundationStiffCohData(~, ~)
        ...
        safeName = sanitizeDatColumnName('动态刚度_相干性');
        defaultName = [safeName '.dat'];
        [saveFile, savePath, filterIndex] = uiputfile( ...
            {'*.dat', 'DAT 文件 (*.dat)'; '*.mat', 'MAT 文件 (*.mat)'; '*.xlsx', 'Excel 文件 (*.xlsx)'}, ...
            '导出地基刚度/相干性数据', defaultName);
        if isequal(saveFile, 0) || isequal(savePath, 0)
            set(lblStatus, 'String', '状态: 已取消导出图窗数据');
            return;
        end
    ...
    function renameSelectedFromField(showSelectionTips)
        ...
        if isempty(newLabel)
            if showSelectionTips
            showAlertCompat(fig, '请先在重命名输入框中填写新名称。', '提示');
            end
            return;
        end
        ...
        if ~isfinite(seriesFileId)
            showAlertCompat(fig, '无法定位所选数据项，请重新加载文件后再试。', '重命名失败');
            return;
        end
        ...
        set(lblStatus, 'String', sprintf('状态: 已将“%s”重命名为“%s”', S.label, renamedLabel));
    end
    ...
    function onScaleEdited(~, ~)
        ...
        if ~isfinite(scaleValue)
            ...
            showAlertCompat(fig, '系数必须是有效数字。', '提示');
            return;
        end
    ...
    function [usedRef, statusMsg] = renderOneAxis(ax, mode, selectedSeries, app, refInput, keepExisting, timeWindow, psdSourceMode, quantityMode)
        ...
        switch mode
            case 'Time'
                ...
                if anyTime
                    title(ax, sprintf('时域图 - %s（%d 条）', quantityName, numel(selectedSeries)));
                    ...
                    title(ax, sprintf('时域图 - %s（无有效数据）', quantityName));
                    legend(ax, 'off');
                end
            ...
            case 'PSD'
                ...
                if anyPsd
                    if usePeriodogramFromTime
                    title(ax, sprintf('PSD - %s（periodogram，对数坐标，%d 条）', quantityName, numel(selectedSeries)));
                    else
                    title(ax, sprintf('PSD - %s（VNA 原始，对数坐标，%d 条）', quantityName, numel(selectedSeries)));
                    end
                    legend(ax, 'show', 'Location', 'northeast');
                    if ~keepExisting && isfinite(xMin) && isfinite(xMax) && xMax > xMin
                        xlim(ax, [xMin, xMax]);
                    ...
                    title(ax, sprintf('PSD - %s（无有效数据）', quantityName));
                    legend(ax, 'off');
                end
            ...
            case 'CumPSD'
                ...
                if anyCum
                    if usePeriodogramFromTime
                        title(ax, sprintf('累计谱 - %s（periodogram，%d 条）', quantityName, numel(selectedSeries)));
                    else
                        title(ax, sprintf('累计谱 - %s（VNA 原始，%d 条）', quantityName, numel(selectedSeries)));
                    end
                    legend(ax, 'show', 'Location', 'northeast');
                    if ~keepExisting && isfinite(xMin) && isfinite(xMax) && xMax > xMin
                        xlim(ax, [xMin, xMax]);
                    ...
                    title(ax, sprintf('累计谱 - %s（无有效数据）', quantityName));
                    legend(ax, 'off');
                end
            ...
            statusMsg = sprintf('传递率已跳过：自身通道 %d，缺少传函 %d', skippedSelfCount, skippedMissingCount);
                end
            ...
            case 'Coherence'
                ...
                ylabel(ax, 'Coherence');
                if anyCoh
                    title(ax, sprintf('相干性（%d 条）', numel(selectedSeries)));
                    legend(ax, 'show', 'Location', 'northeast');
                    if ~keepExisting && isfinite(xMin) && isfinite(xMax) && xMax > xMin
                        xlim(ax, [xMin, xMax]);
                    end
    ...
    function onHoldChanged(~, ~)
        if get(btnHold, 'Value')
            set(lblStatus, 'String', '状态: 保持已开启（下次绘图将追加）');
        else
            set(lblStatus, 'String', '状态: 保持已关闭（下次绘图将覆盖）');
        end
    end
    ...
    function onClearPlots(~, ~)
        ...
        set(lblStatus, 'String', '状态: 图像已清空');
    end

    % 删除选中的数据项，并从对应文件有效通道中移除
    function onDeleteSelectedFiles(~, ~)
        ...
        selectedSeries = getSelectedSeries(app.series, getSelectedListLabels(lstData));
        if isempty(selectedSeries)
        showAlertCompat(fig, '请先在列表中选择要删除的数据项。', '提示');
            return;
        end
        ...
        refreshFoundationFileSelectors();
        cla(axFoundVib); cla(axFoundStiff); cla(axFoundCoh);
        set(lblStatus, 'String', sprintf('状态: 已删除 %d 个数据项，剩余 %d 个', removed, numel(app.series)));
    end
    ...
    function refreshFoundationFileSelectors(preferredVibId, preferredStiffId)
        ...
        if isempty(app.files)
            set(ddVibFile, 'String', {'(空)'}, 'Value', 1, 'Enable', 'off', 'UserData', NaN);
            set(ddStiffFile, 'String', {'(空)'}, 'Value', 1, 'Enable', 'off', 'UserData', NaN);
            return;
        end
        ...
        if vibFallback || stiffFallback
            set(lblStatus, 'String', '状态: 地基页源文件选择已重置为空');
        end
    end
    ...
    function plotFoundationPage(app, keepExisting, silentNoSource)
        ...
        if okVib
            ...
            if ok
                msgParts{end + 1} = sprintf('振动:%s', Fvib.fileName); %#ok<AGROW>
            elseif ~isempty(msg)
                msgParts{end + 1} = ['振动已跳过（' msg '）']; %#ok<AGROW>
            end
        else
            msgParts{end + 1} = '振动文件不可用'; %#ok<AGROW>
        end
        ...
        if okStiff
            ...
            if ok
                if isempty(msg)
                    msgParts{end + 1} = sprintf('刚度:%s', Fstiff.fileName); %#ok<AGROW>
                else
                    msgParts{end + 1} = sprintf('刚度:%s（%s）', Fstiff.fileName, msg); %#ok<AGROW>
                end
            elseif ~isempty(msg)
                msgParts{end + 1} = ['刚度已跳过（' msg '）']; %#ok<AGROW>
            end

            [ok, msg] = renderFoundationCoherenceAxis(axFoundCoh, Fstiff, exciteCh, respChannels, keepExisting);
            if ok
                if isempty(msg)
                    msgParts{end + 1} = sprintf('相干性:%s', Fstiff.fileName); %#ok<AGROW>
                else
                    msgParts{end + 1} = sprintf('相干性:%s（%s）', Fstiff.fileName, msg); %#ok<AGROW>
                end
            elseif ~isempty(msg)
                msgParts{end + 1} = ['相干性已跳过（' msg '）']; %#ok<AGROW>
            end
        else
            msgParts{end + 1} = '刚度文件不可用'; 
            msgParts{end + 1} = '相干性文件不可用'; %#ok<AGROW>
        end

        if isempty(msgParts)
            set(lblStatus, 'String', '状态: 已跳过地基页绘图');
        else
            set(lblStatus, 'String', ['状态: 地基页已绘制 | ' strjoin(msgParts, '，')]);
        end
    end
end
...
if ~isfield(F, 'vna') || ~isfield(F.vna, 'freq') || isempty(F.vna.freq)
    msg = '缺少 fdxvec';
    title(ax, '地面振动（缺少 fdxvec）');
    return;
end
if ~isfield(F.vna, 'aspec') || isempty(F.vna.aspec)
    msg = '缺少 scmeas.aspec';
    title(ax, '地面振动（缺少 aspec）');
    return;
end
rbw = F.vna.rbw;
if ~isfinite(rbw) || rbw <= 0
    msg = 'rbw 无效';
    title(ax, '地面振动（rbw 无效）');
    return;
end
...
hold(ax, 'off');
grid(ax, 'on');
xlabel(ax, '三分之一倍频程中心频率 [Hz]');
ylabel(ax, '均方根速度 [um/s]');
title(ax, sprintf('地面振动 - %s', F.fileName));
...
if ~isfield(F, 'vna') || ~isfield(F.vna, 'freq') || isempty(F.vna.freq)
    msg = '缺少 fdxvec';
    title(ax, '动态刚度（缺少 fdxvec）');
    return;
end
if ~isfield(F.vna, 'xcmeas') || isempty(F.vna.xcmeas)
    msg = '缺少 xcmeas';
    title(ax, '动态刚度（缺少 xcmeas）');
    return;
end
...
if numel(sz) < 2 || exciteCh < 1 || exciteCh > sz(1)
    msg = sprintf('xcmeas 激励通道越界 (%d)', exciteCh);
    title(ax, '动态刚度（激励通道越界）');
    return;
end
if isempty(respChannels)
    msg = '刚度通道为空';
    title(ax, '动态刚度（响应通道为空）');
    return;
end
...
if plottedCount > 0
    ...
    if specF2 > specF1
        safeLoglog(ax, [specF1 specF2], [1e8 1e8], '--', 'LineWidth', 1.5, ...
            'Color', [0.85 0.20 0.20], 'DisplayName', '规范线 (10^8 N/m)');
    end
end
hold(ax, 'off');
...
xlabel(ax, '频率 [Hz]');
ylabel(ax, '幅值 [N/m]');
title(ax, sprintf('动态刚度 - %s', F.fileName));
if plottedCount > 0
    legend(ax, 'show', 'Location', 'northwest');
else
    legend(ax, 'off');
...
if plottedCount == 0
    title(ax, '动态刚度（无有效数据点）');
    if isempty(skippedResp)
        msg = '没有有效刚度数据点';
    else
        msg = ['这些刚度通道没有有效数据点: ' formatChannelList(skippedResp)];
    end
    return;
end
...
if ~isempty(skippedResp)
    msg = ['宸茶烦杩囧垰搴﹂?閬? ' formatChannelList(skippedResp)];
end
ok = true;
end
...
if ~isfield(F, 'vna') || ~isfield(F.vna, 'freq') || isempty(F.vna.freq)
    msg = '缺少 fdxvec';
    title(ax, '相干性（缺少 fdxvec）');
    return;
end
if ~isfield(F.vna, 'xcmeas') || isempty(F.vna.xcmeas)
    msg = '缺少 xcmeas';
    title(ax, '相干性（缺少 xcmeas）');
    return;
end
...
xlabel(ax, '频率 [Hz]');
ylabel(ax, '相干性');
title(ax, sprintf('相干性 - %s', F.fileName));
if plottedCount > 0
    legend(ax, 'show', 'Location', 'northwest');
else
    legend(ax, 'off');
...
if plottedCount == 0
    ...
    if isempty(skippedResp)
        ...
        msg = ['这些刚度通道没有有效相干性数据点: ' formatChannelList(skippedResp)];
    end
    return;
end
...
if ~isempty(skippedResp)
    msg = ['已跳过刚度通道: ' formatChannelList(skippedResp)];
end
...
% 兼容判断是否为 periodogram 来源模式，避免使用 contains
function tf = isPeriodogramSource(psdSourceMode)
tf = false;
if ischar(psdSourceMode)
    tf = ~isempty(strfind(lower(psdSourceMode), 'periodogram')) || ~isempty(strfind(psdSourceMode, '时段计算')); %#ok<STREMP>
end
end
...
quantityMode = normalizeQuantityMode(quantityMode);
...
if n <= 3
    ...
    txt = sprintf('已加载 %d 个文件（最近: %s）', n, getSeriesDisplayFileName(files{end}.fileName));
end
end
...
% 获取数据项基础显示名，优先使用自定义名，否则使用文件名+通道
...
% 根据 fileId + 通道，在当前 series 中查找实际显示名
...
fid = fopen(outPath, 'w');
if fid < 0
    errMsg = sprintf('无法写入文件: %s', outPath);
    return;
end
...
try
    % MATLAB 2016 可尝试 basic 模式（无 Excel 环境的兜底）
    xlswrite(outPath, excelData, 'basic');
    ok = true;
catch ME2
    errMsg = sprintf('写入 XLSX 失败。ActiveX: %s | 普通模式: %s | basic 模式: %s。请检查本机 Excel/Office 组件。', ME0.message, ME1.message, ME2.message);
end
end
...
mode = items{idx};
mode = normalizePlotMode(mode);
end

function mode = normalizePlotMode(mode)
if ~ischar(mode)
    mode = 'Time';
    return;
end
...
function quantityMode = normalizeQuantityMode(quantityMode)
if ~ischar(quantityMode)
    quantityMode = 'Acceleration';
    return;
end
switch strtrim(quantityMode)
    case '加速度'
        quantityMode = 'Acceleration';
    case '速度'
        quantityMode = 'Velocity';
    case '位移'
        quantityMode = 'Displacement';
end
end
...
% 统一弹窗提示，uialert 不可用时回退到 errordlg
...
% 兼容读取数值矩阵，新版本用 readmatrix，旧版本自动回退
