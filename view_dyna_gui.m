...
onResize();
    ...
    % 灏嗘寚瀹氳酱褰撳墠鍐呭澶嶅埗鍒板崟鐙?Figure锛屼究浜庝繚瀛樺浘鐗?
    ...
    function refreshFoundationFileSelectors(preferredVibId, preferredStiffId)
        ...
        if isempty(app.files)
            set(ddVibFile, 'String', {'(绌?'}, 'Value', 1, 'Enable', 'off', 'UserData', NaN);
            set(ddStiffFile, 'String', {'(绌?'}, 'Value', 1, 'Enable', 'off', 'UserData', NaN);
            return;
        end
    ...
    function plotFoundationPage(app, keepExisting, silentNoSource)
        ...
        if okStiff
            ...
            msgParts{end + 1} = '刚度文件不可用'; %#ok<AGROW>
...
% 鑾峰彇鏌愰�氶亾 PSD锛堜紭鍏堜娇鐢ㄦ枃浠跺唴棰戣氨锛屽惁鍒欏洖閫� FFT锛?
...
% 璁＄畻浼犻�掔巼锛堥�氶亾/鍙傝�冮�氶亾锛夊苟杞崲涓?dB
...
if ~isempty(skippedResp)
    msg = ['宸茶烦杩囧垰搴﹂�氶亾: ' formatChannelList(skippedResp)];
end
ok = true;
end
...
% 鑾峰彇宸ョ▼鍗曚綅鎹㈢畻绯绘暟 eu_val锛堢己鐪佷负 1锛?
...
% 鏍规嵁宸插姞杞芥枃浠堕噸寤衡�滄暟鎹」鍒楄〃鈥濓紙鏂囦欢+閫氶亾锛?
...
% 浠?series 鏉＄洰瑙ｆ瀽 fileId锛堝吋瀹规棫瀛楁锛?
...
% 灏嗕袱涓潗鏍囪酱澶嶅埗鍒板悓涓� Figure 鐨?subplot(211/212) 涓?
...
% 鍒ゆ柇鍚戦噺鏄惁鍍忔椂闂磋酱锛堝崟璋冮�掑涓旀闀胯繎浼兼亽瀹氾級
...
% 鍏煎 char/cell/string 鐨勫瓧绗︿覆鍒楄〃杞崲
