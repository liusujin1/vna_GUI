...
% 5. 预览骨架振型并导出 GIF 动画。
    % 应用状态集中保存在 app 中，供所有回调共享当前数据与界面状态。
    app = struct();
    ...
    txtFileHint = uicontrol( ... %#ok<NASGU>
        ...
        'String', '测点表中的每个 PointID 行可直接绑定到一个已加载文件。'); %#ok<NASGU>
    ...
    txtPeakHint = uicontrol( ...
        ...
        'Style', 'text', ...
        'HorizontalAlignment', 'left', ...
        'String', '先在 FRF 图上点频率，再点“应用频率”加入候选。', ...
        'Units', 'normalized', ...
    ...
    % Mode 面板采用像素级重排，减少旧版 MATLAB 下的文字裁切与错位。
    ...
    % 自动找峰：综合 FRF 为主，同时补入各测点/各方向的局部模态峰值。
    ...
    % 候选列表是主驱动入口：选中哪个频率，就切到哪个频率的静态振型。
    ...
    % 在 FRF 图上点选频率时，只回填输入框；真正加入候选需再点 Apply。
    ...
    % 以当前目标频率为中心，从所有有效测点中提取复数振型并做归一化。
    ...
    function refreshFileList()
        if isempty(app.files)
            set(lstFiles, 'String', {'未加载文件'}, 'Value', 1);
            return;
        end
    ...
    function refreshCandidateList(peakFreqs)
        ...
        if isempty(displayFreqs)
            set(lstCandidates, 'String', {'(空)'}, 'Value', 1);
            return;
        end
        selIdx = 1;
    ...
    % FRF 视图既展示综合曲线，也叠加自动峰、手动候选和当前选中频率。
    function refreshFrfAxis()
        ...
        if isfinite(app.activeModeFreq) && app.activeModeFreq > 0
            title(axFrf, sprintf('模态 FRF 候选图 - 当前 %.8g Hz', app.activeModeFreq));
        else
            title(axFrf, '模态 FRF 候选图');
        end
    end

    % 测点骨架图只画聚合后的唯一测点，避免同一 PointID 重复显示。
    function refreshPointAxis()
        ...
        title(axLayout, sprintf('测点骨架图（%d 条连线）', size(lineInfo.validEdges, 1)));
    ...
    % 振型绘制核心：根据相位生成瞬时变形，并可选择是否保留当前视角。
    ...
    % 连续动画通过 timer 驱动；静态图与动画图共用同一套绘制逻辑。
    ...
    % 停止动画时可选是否恢复静态图，以及是否把视角重置回默认 3D 视图。
    ...
    % 用所有有效测点/方向的传函构造一条综合 FRF，作为自动找峰的主参考。
    ...
    % 在对数频率轴上平滑找峰，再回吸附到原始曲线局部最大值，减少偏峰现象。
    ...
    % 补充各单独 FRF 的局部峰，避免只在某个测点明显的模态被平均抹掉。
    ...
    % 当前活动频率切换后，立即清空旧振型并刷新 FRF 标记与候选选中态。
    ...
    % 仅保留启用状态且字段完整的测点行；可按需要要求必须绑定到已加载文件。
    ...
    % 自动连线只负责补草稿，不覆盖手工线；最终结果按去重后的线集输出。
    ...
    % 读取 FRF 后按激励/响应通道工程单位做修正，保持不同文件间量纲一致。
    ...
    % 同一 PointID 的多行数据会在这里聚合，支持“同测点多文件多方向”场景。
    ...
    % 某些 .vna 的频率轴长度与 xfer/coh 长度不完全一致，这里统一对齐。
    ...
    % 解析单个 .vna 文件，只提取当前脚本真正使用到的模态相关字段。
    ...
    % 结构体数组与 uitable 单元格数据之间做双向转换，便于编辑与持久化。
